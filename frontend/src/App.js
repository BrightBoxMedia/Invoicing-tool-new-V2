...
// FIXED Projects Component with Proper Billing Status
const Projects = ({ currentUser }) => {
  ...
  const fetchProjectBilling = async (projectId) => {
    try {
      const token = localStorage.getItem('token');
      ...
      setBillingData(response.data.billing_status || null);
    } catch (error) {
      console.error('Error fetching project billing:', error);
      setBillingData(null);
    }
  };
  ...
  const handleViewBilling = async (project) => {
    if (selectedProject?.id === project.id) {
      ...
      await fetchProjectBilling(project.id);
    }
  };
            ...
            <div className="flex justify-between items-start mb-4">
              <div className="flex-1">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-gray-600">
                  ...
                  <div><strong>Value:</strong> ₹{(project.total_project_value || 0).toLocaleString()}</div>
                </div>
              </div>
              <button
                ...
                {billingData ? (
                  <div className="space-y-4">
                    ...
                    <div className="bg-blue-50 p-4 rounded-lg">
                      <h4 className="font-semibold text-blue-900 mb-2">📊 Billing Summary</h4>
                      <div className="grid grid-cols-4 gap-4 text-sm">
                        <div>
                          <span className="text-blue-600">Next RA:</span>
                          <div className="font-bold text-lg">{billingData.next_ra}</div>
                        </div>
                        <div>
                          <span className="text-blue-600">Total Billed:</span>
                          <div className="font-bold">₹{(billingData.total_billed || 0).toLocaleString()}</div>
                        </div>
                        <div>
                          <span className="text-blue-600">Remaining:</span>
                          <div className="font-bold text-green-600">₹{(billingData.remaining_value || 0).toLocaleString()}</div>
                        </div>
                        <div>
                          <span className="text-blue-600">Completion:</span>
                          <div className="font-bold">{billingData.completion_percentage || 0}%</div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* BOQ Items Table */}
                    <div>
                      <h4 className="font-semibold mb-3">📋 BOQ Items - {billingData.next_ra} Billing</h4>
                      <div className="overflow-x-auto">
                        <table className="min-w-full border rounded-lg">
                          <thead className="bg-gray-100">
                            <tr>
                              <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">Description</th>
                              <th className="px-3 py-3 text-center text-sm font-medium text-gray-700">Unit</th>
                              <th className="px-3 py-3 text-center text-sm font-medium text-gray-700">Total Qty</th>
                              <th className="px-3 py-3 text-center text-sm font-medium text-gray-700">Billed</th>
                              <th className="px-3 py-3 text-center text-sm font-medium text-gray-700">Available</th>
                              <th className="px-3 py-3 text-right text-sm font-medium text-gray-700">Rate</th>
                              <th className="px-3 py-3 text-center text-sm font-medium text-gray-700">Bill Qty</th>
                              <th className="px-3 py-3 text-center text-sm font-medium text-gray-700">GST%</th>
                            </tr>
                          </thead>
                          <tbody>
                            {billingData.boq_items?.map((item, index) => (
                              <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                <td className="px-4 py-3 text-sm">{item.description}</td>
                                <td className="px-3 py-3 text-center text-sm">{item.unit}</td>
                                <td className="px-3 py-3 text-center text-sm font-medium">{item.original_quantity}</td>
                                <td className="px-3 py-3 text-center text-sm text-red-600">{item.billed_quantity}</td>
                                <td className="px-3 py-3 text-center text-sm text-green-600">{item.remaining_quantity}</td>
                                <td className="px-3 py-3 text-right text-sm">₹{(item.rate || 0).toLocaleString()}</td>
                                ...
                                <td className="px-3 py-3 text-center">
                                  <select className="w-16 px-1 py-1 text-sm border border-gray-300 rounded">
                    ...
                    <div className="flex justify-end">
                      <button className="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700">
                        Create {billingData.next_ra} Invoice
                      </button>
                    </div>
                  </div>
                ) : (
                  <div className="text-center py-4 text-gray-500">Loading billing data...</div>
                )}
              </div>
            )}
          </div>
